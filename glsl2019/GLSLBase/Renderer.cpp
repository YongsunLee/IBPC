...
void Renderer::DrawParticleFixedCDTexture()
{
	...
	if (nullptr != m_pFixedCDCamera) {
		GLuint ProjViewId = glGetUniformLocation(shader, "u_ProjView");
		glUniformMatrix4fv(ProjViewId, 1, GL_FALSE, &m_pFixedCDCamera->GetProjView()[0][0]);
	}
...
void Renderer::DrawParticleCDTexture()
{
	...
	if (nullptr != m_pCDCamera) {
		GLuint ProjViewId = glGetUniformLocation(shader, "u_ProjView");
		glUniformMatrix4fv(ProjViewId, 1, GL_FALSE, &m_pCDCamera->GetProjView()[0][0]);
	}
...
{
	glViewport(0, 0, g_ClientWidth/2, g_ClientHeight);
...
void Renderer::DrawObjectFixedCDTexture(GameObject& obj)
{
	auto shader = m_MeshCDShader.get();
	glUseProgram(shader);

	glUniformMatrix4fv(glGetUniformLocation(shader, "u_ProjView"), 1, GL_FALSE, &m_pFixedCDCamera->GetProjView()[0][0]);
	glUniformMatrix4fv(glGetUniformLocation(shader, "u_World"), 1, GL_FALSE, &obj.transform().mtx()[0][0]);
	auto& camera_pos = m_pFixedCDCamera->GetPos();
	glUniform3f(glGetUniformLocation(shader, "u_CameraPos"), camera_pos.x, camera_pos.y, camera_pos.z);
...
void Renderer::DrawObjectCDTexture(GameObject& obj)
{
	...
	glUniformMatrix4fv(glGetUniformLocation(shader, "u_ProjView"), 1, GL_FALSE, &m_pCDCamera->GetProjView()[0][0]);
	glUniformMatrix4fv(glGetUniformLocation(shader, "u_World"), 1, GL_FALSE, &obj.transform().mtx()[0][0]);
	auto& camera_pos = m_pCDCamera->GetPos();
	glUniform3f(glGetUniformLocation(shader, "u_CameraPos"), camera_pos.x, camera_pos.y, camera_pos.z);
...
void Renderer::DrawObject(GameObject& obj)
{
	...
	glVertexAttribPointer(attribPosition, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), 0);
	glVertexAttribPointer(attribNormal, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), (GLvoid*)(sizeof(float) * 3));
	glVertexAttribPointer(attribTexCoord, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex), (GLvoid*)(6 * sizeof(float)));
...
void Renderer::DrawParticleSystem()
{
	...
	DumpTexture(m_FB_ParticleCDTexture[m_CurrTFO].get_rendertarget_buffer(), 500, 0, 500, 500);
	//DumpTexture(m_FB_ObjectCDTexture.get_rendertarget_buffer(1), 250, 0, 250, 250);
...
void Renderer::CreateParticleTFO()
{
	...
	for (unsigned int i = 0; i < m_nTFOs; i++) {
		...
		// -> <- Collision
		for (int j = 0; j < m_pTFOs[i].count / 2; ++j) {
			vertex.pos = glm::vec3(RAND_FLOAT(-0.4f, -0.2f), RAND_FLOAT(0.2, 0.3) , RAND_FLOAT(0.4, 0.6));
			//vertex.dir = glm::vec3(RAND_FLOAT(-1, 1), RAND_FLOAT(1, 5), RAND_FLOAT(-1, 1));
			vertex.dir = glm::vec3(0.3f, 0, 0);
			vertex.speed = 1.0f;
			particles.push_back(vertex);
		}

		for (int j = m_pTFOs[i].count / 2; j < m_pTFOs[i].count; ++j) {
			vertex.pos = glm::vec3(RAND_FLOAT(0.2f, 0.4f), RAND_FLOAT(0.2, 0.3), RAND_FLOAT(0.4, 0.6));
			//vertex.dir = glm::vec3(RAND_FLOAT(-1, 1), RAND_FLOAT(1, 5), RAND_FLOAT(-1, 1));
			vertex.dir = glm::vec3(-0.3f, 0, 0);
			vertex.speed = 1.0f;;
			particles.push_back(vertex);
		...
		for (int j = 0; j < m_pTFOs[i].count; ++j) {
			vertex.pos = glm::vec3(RAND_FLOAT(-0.2f, 0.2f), RAND_FLOAT(-0.2f, 0.2f), RAND_FLOAT(-0.2f, 0.2f));
			vertex.dir = glm::vec3(RAND_FLOAT(-0.2, 0.2), RAND_FLOAT(0.1, 0.5), RAND_FLOAT(-0.2, 0.2));
			vertex.speed = RAND_FLOAT(0.1f, 0.5f);
			particles.push_back(vertex);
		}
		...
		for (int j = 0; j < m_pTFOs[i].count; ++j) {
			...
			vertex.pos = glm::vec3(RAND_FLOAT(0.02f, 0.06f), RAND_FLOAT(0.0f, 20.0f), RAND_FLOAT(-0.05f, 0.05f));
		...
		for (int j = 0; j < m_pTFOs[i].count; ++j) {
			...
			vertex.pos = glm::vec3(RAND_FLOAT(-0.125f, 0.125f), RAND_FLOAT(0.0f, 20.0f), RAND_FLOAT(-0.05f, 0.05f));
			vertex.dir = glm::vec3(0, -1.0f, 0);
			vertex.speed = 0.0f;
			particles.push_back(vertex);
		}
		*/
		...
		// Just Particle Collision Density Type
		//for (int j = 0; j < m_pTFOs[i].count; ++j) {
		//	vertex.pos = glm::vec3(RAND_FLOAT(-0.0525f, 0.0925f), RAND_FLOAT(0.0f, 20.0f), RAND_FLOAT(-0.05f, 0.05f));
		//	vertex.dir = glm::vec3(0, -1.0f, 0);
		//	vertex.speed = 0.0f;
		//	particles.push_back(vertex);
		//}
...
void Renderer::CreateSceneObjects()
{
	...
	dragon.transform().set_pos(glm::vec3(-0.0f, -0.45f, 0));
