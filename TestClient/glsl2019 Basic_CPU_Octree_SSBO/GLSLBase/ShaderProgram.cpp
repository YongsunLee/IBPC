...
void ShaderProgram::compile(const std::string& file_path_comp)
{
	ResetProgram();
	...
	if (false == ReadFile(vs, file_path_comp)) {
		printf("Error compiling compute shader\n");
		return;
	};

	// ShaderProgram 에 vs.c_str() 버텍스 쉐이더를 컴파일한 결과를 attach함
	AddShader(mHandle, vs, GL_COMPUTE_SHADER);
	...
	if (GL_FALSE == success) {
		// shader program 로그를 받아옴
		glGetProgramInfoLog(mHandle, sizeof(ErrorLog), NULL, ErrorLog);
		std::cout << file_path_comp << " Error linking shader program\n" << ErrorLog;
		return;
	}

	glUseProgram(mHandle);
	std::cout << file_path_comp << " Shader compiling is done.\n";
}
